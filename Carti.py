...
class Books:
    ...
    def adauga(self, ISBN, Title, ApparitionDay, Writer):
        self.cursor.execute(f"INSERT INTO books (ISBN, Title, ApparitionDay, Writer) VALUES ('{ISBN}', '{Title}', '{ApparitionDay}', '{Writer}')")
        self.conn.commit()
    def show(self):
    ...
    def update(self, id, ISBN, Title, ApparitionDay, Writer):
        self.cursor.execute(f"UPDATE books SET ISBN = {ISBN}, Title = '{Title}', ApparitionDay = {ApparitionDay}, Writer = {Writer} WHERE id = {id}")
        self.conn.commit()
book = Books()
